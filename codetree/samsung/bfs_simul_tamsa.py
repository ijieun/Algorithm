        ...
        for i in range(N_large):
            for j in range(N_large):
                if not visit[i][j]:
                    ...
                    # 이때 trace_q 안에 조각들의 위치가 저장됩니다.
                    bfs_q, trace_q = deque([(i, j)]), deque([(i, j)])
                    visit[i][j] = True
                    while bfs_q:
                        cur = bfs_q.popleft()
                        for k in range(4):
                            new_y, new_x = cur[0] + dy[k], cur[1] + dx[k]
                            if self.in_range(new_y, new_x) and self.a[new_y][new_x] == self.a[cur[0]][cur[1]] and not visit[new_y][new_x]:
                                bfs_q.append((new_y, new_x))
                                trace_q.append((new_y, new_x))
